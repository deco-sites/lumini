        ...
        <section class="w-full h-full flex items-center justify-center my-2">
            <div class="max-w-[1536px] h-full flex flex-col items-start justify-start mx-auto mt-4">
                <div class="w-full h-full my-4">
                    <p dangerouslySetInnerHTML={{ __html: title ?? "" }} class="text-[40px] text-[#3f3f40] my-6"/>
                </div>
                <div class="w-full h-full flex flex-wrap items-center justify-start gap-5">
                    {cards?.map((card) => (
                        
                        <div class="w-[498px] h-full flex flex-col">
                            <div class="w-full h-full">
                                <Image src={card.image.linkImage} width={376} height={238} alt={card.image.descriptionImage} />
                            </div>
                            <div class="my-2">
                                <p dangerouslySetInnerHTML={{ __html: card.city ?? "" }} class="text-2xl font-semibold my-2"/>

                                <p dangerouslySetInnerHTML={{ __html: card.description ?? "" }} class="text-base my-2"/>
                            </div>
